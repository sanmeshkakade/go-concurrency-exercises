...
func (m *SessionManager) EvictSessions() {
	ticker := time.Tick(time.Second * 5)

	for {
		<-ticker
		m.mutex.Lock()
		for sessionId, sesson := range m.sessions {
			if sesson.EvictionTime.Before(time.Now()) {
...
func (m *SessionManager) CreateSession() (string, error) {
	...
	m.sessions[sessionID] = Session{
		...
		EvictionTime: time.Now().Add(time.Second * 5),
	}
...
func (m *SessionManager) UpdateSessionData(sessionID string, data map[string]interface{}) error {
	...
	m.sessions[sessionID] = Session{
		Data:         data,
		EvictionTime: time.Now().Add(5 * time.Second),
	}
